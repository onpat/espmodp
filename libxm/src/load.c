...
void xm_dump_context(xm_context_t* __restrict ctx, char* __restrict out) {
	...
	__attribute__((unused)) uint64_t old_hash = xm_fnv1a((void*)ctx, ctx_size);
...
void xm_save_context(const xm_context_t* __restrict ctx, char* __restrict out) {
...
static uint32_t xm_load_xm0104_instrument(xm_context_t* ctx,
	...
	__attribute__((unused)) uint32_t extra_samples_size = 0;
...
static void xm_load_mod(xm_context_t* __restrict ctx,
	...
	for(uint8_t i = 0; i < ctx->module.num_samples; ++i) {
		#if HAS_INSTRUMENTS
		__attribute__((unused)) xm_instrument_t* ins = ctx->instruments + i;
		#endif
