...
void xm_set_max_loop_count(__attribute__((unused)) xm_context_t* context,
                           __attribute__((unused)) uint8_t loopcnt) {
	#if XM_LOOPING_TYPE == 2
	context->module.max_loop_count = loopcnt;
	#endif
}

uint8_t xm_get_loop_count(__attribute__((unused)) const xm_context_t* context) {
...
bool xm_mute_channel(__attribute__((unused)) xm_context_t* ctx,
                     uint8_t channel,
                     __attribute__((unused)) bool mute) {
...
#else
const char* xm_get_module_name(__attribute__((unused)) const xm_context_t* ctx) {
	return "";
}

const char* xm_get_tracker_name(__attribute__((unused)) const xm_context_t* ctx) {
	return "";
}

const char* xm_get_instrument_name(__attribute__((unused)) const xm_context_t* ctx,
                                   __attribute__((unused)) uint8_t i) {
	return "";
}

const char* xm_get_sample_name(__attribute__((unused)) const xm_context_t* ctx,
...
uint8_t xm_get_number_of_channels(__attribute__((unused)) const xm_context_t* ctx) {
...
uint32_t xm_get_latest_trigger_of_channel(__attribute__((unused)) const xm_context_t* c,
                                          uint8_t chn) {
...
void xm_reset_context(xm_context_t* ctx) {
	...
	for(__typeof__(ctx->module.num_instruments) i = ctx->module.num_instruments;
	    i; --i, ++inst) {
		inst->latest_trigger = 0;
	}

	xm_sample_t* smp = ctx->samples;
	for(__typeof__(ctx->module.num_samples) i = ctx->module.num_samples;
	    i; --i, ++smp) {
...
void xm_set_sample_rate(__attribute__((unused)) xm_context_t* ctx,
...
uint16_t xm_get_sample_rate(__attribute__((unused)) const xm_context_t* ctx) {
	return CURRENT_SAMPLE_RATE(ctx);
}

/* For debugging */
void xm_print_pattern(__attribute__((unused)) xm_context_t* ctx,
                      __attribute__((unused)) uint8_t pat) {
	#if XM_VERBOSE
