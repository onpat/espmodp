...
static void append_str(char* __restrict, uint16_t*, const char* __restrict);
static void append_u16(char*, uint16_t*, uint16_t);
static void append_u64(char*, uint16_t*, uint64_t);

__attribute__((unused)) static uint8_t FEATURE_WAVEFORM(uint8_t);
...
static void append_str(char* __restrict dest, uint16_t* dest_offset,
...
static void analyze_note_trigger(xm_context_t* ctx, xm_channel_context_t* ch,
	...
	__attribute__((unused)) xm_sample_t* smp;
...
void xm_analyze(xm_context_t* __restrict ctx, char* __restrict out) {
