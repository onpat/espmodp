...
packed_data_t *encode(int num_samples, int num_channels, int sample_rate, int16_t **channels) {
  ...
  packed_data_t *p_data = (packed_data_t *)std::malloc(sizeof(packed_data_t));
  if (p_data) {
    new (p_data) packed_data_t(encoded_size, num_samples, num_channels, sample_rate, SAC_FORMAT_DD4A);
    if (!p_data->data()) {
      p_data->~packed_data_t();
      std::free(p_data);
      p_data = NULL;
    }
  }
  ...
  scoped_ptr<packed_data_t> data(p_data);
...
packed_data_t *encode_cb(int num_samples, int num_channels, int sample_rate, sac_read_block_cb read_cb, void *user_data) {
  // Calculate encoded size.
  const int num_full_blocks = num_samples / kBlockSize;
  const int final_samples = num_samples - num_full_blocks * kBlockSize;
  const int encoded_size = num_channels * (num_full_blocks * kBytesPerBlock + block_size_in_bytes(final_samples));
  ...
  encoder_t encoder;
  int16_t block_buf[32];

  // Encode all the full blocks.
  for (int k = 0; k < num_full_blocks; ++k) {
    uint8_t *dst = data->data() + k * num_channels * kBytesPerBlock;
    for (int ch = 0; ch < num_channels; ++ch) {
      read_cb(user_data, ch, k * kBlockSize, kBlockSize, block_buf);
      encoder.encode_block(block_buf, dst, kBlockSize, 1);
      dst += kBytesPerBlock;
    }
  }
  ...
  if (final_samples > 0) {
    uint8_t *dst = data->data() + num_full_blocks * num_channels * kBytesPerBlock;
    for (int ch = 0; ch < num_channels; ++ch) {
      read_cb(user_data, ch, num_full_blocks * kBlockSize, final_samples, block_buf);
      encoder.encode_block(block_buf, dst, final_samples, 1);
      dst += block_size_in_bytes(final_samples);
    }
  }
