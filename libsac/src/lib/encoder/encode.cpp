...
extern "C"
sac_packed_data_t *sac_encode_cb(int num_samples, int num_channels, int sample_rate, sac_encoding_t format, sac_read_block_cb read_cb, void *user_data) {
  // Check input arguments
  if (!read_cb || num_channels < 1 || num_samples < 1 || sample_rate < 1 || (format != SAC_FORMAT_DD4A && format != SAC_FORMAT_DD8A)) {
    return 0;
  }
  ...
  switch (format) {
    case SAC_FORMAT_DD4A:
      out = dd4a::encode_cb(num_samples, num_channels, sample_rate, read_cb, user_data);
      break;
    case SAC_FORMAT_DD8A:
      out = dd8a::encode_cb(num_samples, num_channels, sample_rate, read_cb, user_data);
      break;
    default:
      break;
  }
